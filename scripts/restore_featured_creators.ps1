...
$searchRegex = '(?s)        <option>Tech</option>.*?<p style="margin: 0; min-width: 250px;">Step-by-step guide to achieving your campaign goals.</p>'
...
$replace = @"
  ...
  <section class="container section animate-on-scroll">
    <div class="flex items-center gap-8 mb-8" style="flex-wrap: wrap;">
      ...
      <p style="margin: 0; min-width: 250px;">Step-by-step guide to achieving your campaign goals.</p>
"@

$html = [regex]::Replace($html, $searchRegex, $replace)

[System.IO.File]::WriteAllText("f:\Website\index.html", $html, (New-Object System.Text.UTF8Encoding $false))
Write-Output "Successfully restored the missing featured creators block in index.html!"
