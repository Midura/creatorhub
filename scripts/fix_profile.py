...
with open("f:/Website/profile.html", "r", encoding="utf-8") as f:
    content = f.read()

# The file got butchered. Let's look for the remaining parts to stitch it back together.
start_idx = content.find('<li style="color: var(--white); opacity: 0.8; font-size: 0.875rem;">')
end_idx = content.find('<!-- YouTube SVG -->')

if start_idx != -1 and end_idx != -1:
    before = content[:start_idx]
    # Keep the first part of the ul
    after = content[end_idx:]
    
    # We need to backtrack the end_idx to the <a href...> tag before the youtube SVG
    end_idx_a = content.rfind('<a href="#"', start_idx, end_idx)

    if end_idx_a != -1:
        after = content[end_idx_a:]

    insertion = """<li style="color: var(--white); opacity: 0.8; font-size: 0.875rem;">&#128205; Colombo Trade City, Colombo 03</li>
    ...
    new_content = before + insertion + after[after.find('<a href'):]
    
    with open("f:/Website/profile.html", "w", encoding="utf-8") as f:
        f.write(new_content)
